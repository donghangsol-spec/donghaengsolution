...
async function call(path,token,payload){const base=process.env.SUPABASE_URL,key=process.env.SUPABASE_ANON_KEY;if(!base||!key)throw new Error('server config missing');const r=await fetch(base+path,{method:'POST',headers:{apikey:key,Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(payload)});const text=await r.text();if(!r.ok)throw Object.assign(new Error('request failed'),{status:r.status,detail:text});return text?JSON.parse(text):null;}
async function service(path,payload){const base=process.env.SUPABASE_URL,key=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!base||!key)throw new Error('server config missing');const r=await fetch(base+path,{method:'POST',headers:{apikey:key,Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify(payload)});const text=await r.text();if(!r.ok)throw Object.assign(new Error('commit failed'),{status:r.status,detail:text});return text?JSON.parse(text):null;}
export default async function handler(req,res){
 ...
 try{
   // Authenticated INVOKER review RPC proves row visibility + reviewer role before privileged commit.
   // Re-approving an already approved draft fails closed, so client must approve immediately before commit.
   await call('/rest/v1/rpc/review_intake_work_draft',token,{p_id:id,p_decision:'approved'});
