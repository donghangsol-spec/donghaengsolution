...
async function sb(path,token,method='GET',payload){const base=process.env.SUPABASE_URL,key=process.env.SUPABASE_ANON_KEY;if(!base||!key)throw new Error('server config missing');const r=await fetch(base+path,{method,headers:{apikey:key,Authorization:'Bearer '+token,...(payload?{'Content-Type':'application/json'}:{})},body:payload?JSON.stringify(payload):undefined});const text=await r.text();if(!r.ok)throw Object.assign(new Error('request failed'),{status:r.status});return text?JSON.parse(text):null;}
...
export default async function handler(req,res){
 ...
 try{
   ...
   let allowed;try{allowed=await sb('/rest/v1/rpc/authorize_intake_work_commit',token,'POST',{p_id:id});}catch(e){if(e.status===401||e.status===403)return res.status(403).json({ok:false,reason:'forbidden'});throw e}
   if(allowed!==true)return res.status(403).json({ok:false,reason:'forbidden'});
   const user=await sb('/auth/v1/user',token);if(!user?.id)return res.status(401).json({ok:false});
   const entity=await service('/rest/v1/rpc/commit_intake_work_draft',{p_id:id,p_actor_id:user.id});
