...
begin
  ...
  update public.insurance_requests
  set status = '승인완료', approved_by = auth.uid(), approved_at = now(), updated_at = now()
...
-- 브라우저에서 상태/승인 컬럼을 직접 바꾸는 우회를 막는다.
-- 요청 상태 변경은 위 SECURITY DEFINER RPC만 수행한다.
revoke update on public.insurance_requests from authenticated;
...
create trigger trg_audit_insurance_requests after insert or update or delete on public.insurance_requests
for each row execute function private.audit_insurance_changes();
